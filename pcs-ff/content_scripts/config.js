...
window.addEventListener('script1Loaded', function () {
    ...
    // var variableDiv, variables;
    // var paperName,
    //     anchor,
    //     searchTerm,
    //     resultsNumberContainer,
    //     resultsNumber,
    //     resultsNumberPerPageDef;
    // var pagesNumber,
    //     articleListDef,
    //     nextPageDef,
    //     pageParam,
    //     nextButtonDef,
    //     aboBtnDef;
    // var premiumBannerDef,
    //     articleHeaderDef,
    //     titleDivDef,
    //     subhedDivDef,
    //     bodyDivDef;
    // var authorElementDef,
    //     dateLogic,
    //     dateElementDef,
    //     dateAttributeDef,
    //     dateStringDef;
    // var textElementsDef, exclElementsText, exclElementsDef;
    ...
    var anchorDef = variables.anchorDef;
    ...
    var articlesDef = variables.articlesDef;
    ...
    if ((paperName === 'Süddeutsche Zeitung')) {
        const logo = document.querySelector('svg.custom-gth0vq')
        const logo2 = logo.cloneNode(true);
        logo2.setAttribute('fill', '#fff');
        legend.appendChild(logo2);
        legendText.style.display = 'inline';
        legendText.style.verticalAlign = 'top';
        legendText.textContent = ' corpus scraper';
    } else {
                ...
                if (response.success) {
                    ...
                    if (response.skippedFiles.length >= 1) {
                        ...
                        skippedFilesContainer.appendChild(
                            showSkippedListButton
                        );
                        skippedFilesContainer.appendChild(
                            hideSkippedListButton
                        );
                        ...
                        for (let i = 0; i < skippedArticleList.length; i++) {
                            ...
                            try {
                                ...
                                skippedArticleLink.classList.add(
                                    'skipped-link'
                                );
                                ...
                                skippedArticleItem.appendChild(
                                    skippedArticleLink
                                );
                                if (skippedArticleUrl.startsWith('http:')) {
                                    skippedArticleUrl =
                                        skippedArticleUrl.replace(
                                            'http',
                                            'https'
                                        );
                                }
                        ...
                        showSkippedListButton.addEventListener(
                            'click',
                            function () {
                                skippedFilesLinksContainer.style.display =
                                    'block';
                    ...
                    if (response.errorFiles.length >= 1) {
                        ...
                        for (let i = 0; i < errorArticleList.length; i++) {
                            console.log(
                                'Error file URL: ' + errorArticleList[i]
                            );
                        ...
                        errorFilesContainer.appendChild(
                            errorFilesLinksContainer
                        );

                        showErrorListButton.addEventListener(
                            'click',
                            function () {
                                errorFilesLinksContainer.style.display =
                                    'block';
                                hideErrorListButton.style.display = 'block';
                                showErrorListButton.style.display = 'none';
                            }
                        );
                        hideErrorListButton.addEventListener(
                            'click',
                            function () {
                                errorFilesLinksContainer.style.display = 'none';
                                hideErrorListButton.style.display = 'none';
