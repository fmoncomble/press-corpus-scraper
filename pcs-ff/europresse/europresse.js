console.log('Europresse PCS script injected');
updateRange();
...
const paperName = 'Europresse';

// Désigner le point d'insertion de l'encadré de l'extension
const anchor = document.querySelector('div.header-info');
...
const searchTermContainerDef = 'div.input-container input';
...
const resultsNumberContainer = document.querySelector(
    'span.resultOperations-count'
);
console.log('Number of results displayed in: ', resultsNumberContainer);
const resultsNumber = Number(
    resultsNumberContainer.textContent.replaceAll(/\s+/g, '').trim()
);
// Construction de l'URL des pages de recherche
let domain = window.location.origin;
console.log('Domain = ', domain);
let searchURL = domain + '/Search/GetPage?docPerPage=50&pageNo=0';
...
const articleListDef = 'body';
// Elément contenant chaque résultat
const articlesDef = 'div.docListItem';
// Logique de pagination : si les pages sont numérotées dans l'URL (parfois seulement à partir de la 2e page de résultats, ex. https://www.journal.fr/recherche/?keywords=termes%20de%20recherche&page=2), indiquer 'true', sinon 'false' (sans guillemets), puis indiquer le nom du paramètre ('page', 'p' ou autre) et le numéro de la première page de résultats
...
const premiumBannerDef = null;
// Si la bannière se situe dans l'en-tête d'article : élément de l'en-tête
const articleHeaderDef = null;
// Elément du titre de la source
const pubNameDef = 'span.DocPublicationName';
// Elément du titre de l'article
const titleDivDef = 'div.titreArticle';
// Elément du lien vers l'article
const euroLinkDef = 'button#btLink';
const euroLinkAttribute = 'data-clipboard-text';
// Elément du chapô
const subhedDivDef = null;
// Elément du corps de l'article
const bodyDivDef = 'div.docOcurrContainer';
// Elément du nom de l'auteur.e
const authorElementDef = 'p.sm-margin-bottomNews';
// Logique de date :
...
if (pagesTotal > 20) {
    label.textContent = `${chrome.i18n.getMessage(
        'europresseLabelText1'
    )}${pagesTotal}`;
} else {
    label.textContent = `${chrome.i18n.getMessage(
        'extractLabelText1'
    )}${pagesTotal}${chrome.i18n.getMessage('extractLabelText2')}`;
}
        ...
        function respond(msg) {
            if (msg) {
                ...
                extractionMessage.textContent = `${chrome.i18n.getMessage(
                    'extractionMessage2'
                )}${msg.sentPageNo}${chrome.i18n.getMessage(
                    'extractionMessage3'
    ...
    chrome.runtime.sendMessage(
        ...
        (response) => {
            ...
            if (response.success) {
                ...
                )}\n\n${pageNo}${chrome.i18n.getMessage(
                    'downloadFilesContainerText2'
                ...
                if (response.skippedFiles.length >= 1) {
                    ...
                    if (response.skippedFiles.length === 1) {
                        ...
                        skippedFilesWrapper.textContent = `\n${
                            response.skippedFiles.length
                        }${chrome.i18n.getMessage(
                            'skippedFilesWrapperText2'
                        )}\n\n`;
                    }
